...
namespace sinensis
{
	...
	public:
		...
		const T* cbegin() const { return _memoryBlock.get(); }
		
		const T* cend() const { return _memoryBlock.get() + _size; }
		...
		void shrink_to_fit()
		{
			std::unique_ptr<T[]> newMemoryBlock = std::make_unique<T[]>(_size);
		...
		T* insert(const T* position, std::size_t count, const T& value)
		{
			...
			std::size_t j = 0;
			std::unique_ptr<T[]> newMemoryBlock = std::make_unique<T[]>(_capacity);
		...
		std::size_t erase_if(const T* start, const T* finish, const T& value)
		{
			...
			std::unique_ptr<std::size_t[]> remove_indices = std::make_unique<std::size_t[]>(dist);
